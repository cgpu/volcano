...
if (!require("Biobase"))BiocManager::install("Biobase" , ask = FALSE , update = FALSE)
if (!require("CLL"))BiocManager::install("CLL"         , ask = FALSE , update = FALSE)
...
# The outcome or y or predictor variable is the `Disease` status `{stable, progressing}` 
# (binary outcome, aka control case)
...
# With FDR set to `r FDR_threshold` (soft-coded to inherit `FDR_threshold` value set above), 
# `r `  differentially regulated features found 
# between patients with `(Disease)stable` and `progres` subject of teh experiment for the examined contrast ()
...
write.table( toPlot,  
           file        = paste0(here::here(), "/example_toPlot_tibble.csv"),
             append    = FALSE, 
             quote     = FALSE, 
             sep       = ",",
             row.names = F,
...
FILENAME <- paste0(here::here(),"/plotly_volcano.html")
...
# Custom colours
ggtitle_custom_color       <- webPalette("grey dark")[[1]] # webPalette("grey dark") "#4A637B"
ggsubtitle_custom_color    <- webPalette("grey light")[[1]] # "#A9BACA" # webPalette("grey medium")
dashed_line_color          <- "#990000" # wine red
scale_color_manual_palette <- c(webPalette("cta blue")[[1]],
                                webPalette("watermelon fail")[[1]], 
                                webPalette("grey dark")[[1]] ) 
transparency               <- 0.70
...
htmlwidgets::saveWidget(ggplotly(p), file = FILENAME)
